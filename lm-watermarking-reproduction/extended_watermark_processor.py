...
class WatermarkBase:#水印基础类
    ...
    def _initialize_seeding_scheme(self, seeding_scheme: str) -> None:#初始化种子生成策略的内部设置
    ...
    def _seed_rng(self, input_ids: torch.LongTensor) -> None:#随机数生成器
        ...
        if input_ids.shape[-1] < self.context_width:#检查上下文长度
            raise ValueError(f"seeding_scheme requires at least a {self.context_width} token prefix to seed the RNG.")

        prf_key = prf_lookup[self.prf_type](input_ids[-self.context_width :], salt_key=self.hash_key)#生成prf密钥，取最后context_width个令牌作为上下文
        # enable for long, interesting streams of pseudorandom numbers: print(prf_key)
        self.rng.manual_seed(prf_key % (2**64 - 1))  # safeguard against overflow from long，设置随机数生成种子

    def _get_greenlist_ids(self, input_ids: torch.LongTensor) -> torch.LongTensor:#生成绿色令牌列表
        """Seed rng based on local context width and use this information to generate ids on the green list."""
        self._seed_rng(input_ids)#设置随机数生成器

        greenlist_size = int(self.vocab_size * self.gamma)#绿名单大小
        vocab_permutation = torch.randperm(self.vocab_size, device=input_ids.device, generator=self.rng)#对词表进行随机排列
        if self.select_green_tokens:  # directly
            greenlist_ids = vocab_permutation[:greenlist_size]  # new，前size个为绿名单
        else:  # select green via red
            greenlist_ids = vocab_permutation[(self.vocab_size - greenlist_size) :]  # legacy behavior#剩下的为绿名单
        return greenlist_ids


class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):#修改模型输出分数
    ...
    def _init_spike_entropies(self):#计算尖峰熵的相关参数
        alpha = torch.exp(torch.tensor(self.delta)).item()
        gamma = self.gamma

        self.z_value = ((1 - gamma) * (alpha - 1)) / (1 - gamma + (alpha * gamma))#计算z值((1-γ)(α-1))/(1-γ+αγ)
        self.expected_gl_coef = (gamma * alpha) / (1 - gamma + (alpha * gamma))#计算期望的绿色列表系数

        # catch for overflow when bias is "infinite"
        if alpha == torch.inf:#处理无穷大的情况
            self.z_value = 1.0
    ...
    def _compute_spike_entropy(self, scores):#计算尖峰熵值
        # precomputed z value in init
        probs = scores.softmax(dim=-1)#将分数转换为概率分布
        denoms = 1 + (self.z_value * probs)#重归一化
        renormed_probs = probs / denoms
        sum_renormed_probs = renormed_probs.sum()
        return sum_renormed_probs#返回重归一化概率的总和

    def _calc_greenlist_mask(self, scores: torch.FloatTensor, greenlist_token_ids) -> torch.BoolTensor:#计算绿色令牌掩码
        # Cannot lose loop, greenlists might have different lengths
        green_tokens_mask = torch.zeros_like(scores, dtype=torch.bool)#创建全零的布尔张量
        for b_idx, greenlist in enumerate(greenlist_token_ids):
            if len(greenlist) > 0:
                green_tokens_mask[b_idx][greenlist] = True#为绿色令牌位置设为true
        return green_tokens_mask

    def _bias_greenlist_logits(self, scores: torch.Tensor, greenlist_mask: torch.Tensor, greenlist_bias: float) -> torch.Tensor:
        scores[greenlist_mask] = scores[greenlist_mask] + greenlist_bias#每个位置添加偏置
        return scores
    ...
    def _score_rejection_sampling(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, tail_rule="fixed_compute") -> list[int]:
        ...
        """
        #模拟如果选择了某个候选令牌，它在扩展后的上下文中是否仍然是绿色的
        sorted_scores, greedy_predictions = scores.sort(dim=-1, descending=True)#按token排序

        final_greenlist = []#自洽性检查的令牌ID
        for idx, prediction_candidate in enumerate(greedy_predictions):
            ...
            if prediction_candidate in greenlist_ids:  # test for consistency
                final_greenlist.append(prediction_candidate)#检查候选令牌是否在基于扩展上下文生成的绿色列表中

            # What follows below are optional early-stopping rules for efficiency
            #提前停止规则方法
            if tail_rule == "fixed_score":#分数差距太大，后面的令牌几乎不可能被选中
                if sorted_scores[0] - sorted_scores[idx + 1] > self.delta:
                    break
            elif tail_rule == "fixed_list_length":#达到列表长度
                if len(final_greenlist) == 10:
                    break
            elif tail_rule == "fixed_compute":#达到计算次数
                if idx == 40:
                    break
            else:
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        self.rng = torch.Generator(device=input_ids.device) if self.rng is None else self.rng#初始化随机数生成器
        ...
        for b_idx, input_seq in enumerate(input_ids):#对batch中每个序列分别处理
            if self.self_salt:#生成绿色令牌列表
                greenlist_ids = self._score_rejection_sampling(input_seq, scores[b_idx])
            else:
                greenlist_ids = self._get_greenlist_ids(input_seq)
            list_of_greenlist_ids[b_idx] = greenlist_ids#计算绿色令牌掩码
        ...
        green_tokens_mask = self._calc_greenlist_mask(scores=scores, greenlist_token_ids=list_of_greenlist_ids)#为绿色令牌添加偏置
        scores = self._bias_greenlist_logits(scores=scores, greenlist_mask=green_tokens_mask, greenlist_bias=self.delta)#返回修改后的分数

        return scores


class WatermarkDetector(WatermarkBase):#检测文本中的水印
    ...
    * z_threshold -> Changing this threshold will change the sensitivity of the detector.
    ...
    def __init__(
        ...
        *args,
        ...
        z_threshold: float = 4.0,#阈值（默认4.0）
    ...
    def dummy_detect(#返回空检测结果（用于测试或占位）
        self,
    ...
    def _compute_z_score(self, observed_count, T):#计算z分数（标准分数）
        # count refers to number of green tokens, T is total number of tokens
        expected_count = self.gamma
        numer = observed_count - expected_count * T#观察值减去期望值
        denom = sqrt(T * expected_count * (1 - expected_count))#标准差
        z = numer / denom
        return z

    def _compute_p_value(self, z):#根据z分数计算p值
        p_value = scipy.stats.norm.sf(z)
        return p_value

    @lru_cache(maxsize=2**32)#使用LRU缓存避免重复计算
    def _get_ngram_score_cached(self, prefix: tuple[int], target: int):#带缓存的n-gram分数计算
        ...
        return True if target in greenlist_ids else False#对于给定的前缀和目标令牌，判断目标是否在绿色列表中

    def _score_ngrams_in_passage(self, input_ids: torch.Tensor):# 收集文本中所有n-gram并计算水印分数
        """Core function to gather all ngrams in the input and compute their watermark."""
        if len(input_ids) - self.context_width < 1:# 确保文本长度足够进行检测
            raise ValueError(
        ...
        frequencies_table = collections.Counter(token_ngram_generator)#统计每个唯一n-gram的出现次数
    ...
    def _get_green_at_T_booleans(self, input_ids, ngram_to_watermark_lookup) -> tuple[torch.Tensor]:
        ...
        for idx, ngram_example in enumerate(ngram_examples):#遍历所有n-gram，记录每个目标令牌是否为绿色
    ...
    def _score_windows_impl_batched(#使用滑动窗口计算z分数（批处理版本）
        self,
        input_ids: torch.Tensor,
        window_size: str,
        window_stride: int = 1,
    ...
    def _score_sequence_window(#使用滑动窗口计算z分数
        self,
        input_ids: torch.Tensor,
        return_num_tokens_scored: bool = True,
        return_num_green_tokens: bool = True,
    ...
    def detect(#检测接口
        ...
        **kwargs,
        ...
        for normalizer in self.normalizers:#应用文本规范化器
            text = normalizer(text)
        if len(self.normalizers) > 0:
            print(f"Text after normalization:\n\n{text}\n")

        if tokenized_text is None:#将文本转换为token ID
...
def ngrams(sequence, n, pad_left=False, pad_right=False, pad_symbol=None):#辅助函数，生成序列的n-gram
